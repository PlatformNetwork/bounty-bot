...
describe('detection/duplicate', () => {
  ...
  describe('generateFingerprint', () => {
    ...
    it('different input = different output', () => {
      const a = generateFingerprint('The quick brown fox jumps over the lazy dog');
      const b = generateFingerprint('A completely different sentence about cats and mice');
      expect(a).not.toBe(b);
    });
  ...
  describe('computeSimilarity', () => {
    it('identical text = 1.0', () => {
      const text = 'Login page crashes when entering special characters in password field';
      const sim = computeSimilarity('', '', text, text);
      expect(sim).toBe(1.0);
    });

    it('completely different text = low score', () => {
      const text1 = 'Login page crashes when entering special characters password field rendering';
      const text2 = 'Database migration script timeout kubernetes deployment configuration orchestration';
  ...
  describe('findDuplicates', () => {
    ...
    it('similar older issue -> duplicate detected', async () => {
      const olderText = 'Login page crashes when entering special characters in the password field on Chrome browser';
      upsertEmbedding({
        issue_number: 42000,
    ...
    it('similar newer issue (higher number) -> NOT flagged', async () => {
      upsertEmbedding({
        ...
        body_fingerprint: 'Same bug Login page crashes when entering special characters',
      });
    ...
    it('issue below ISSUE_FLOOR -> ignored as candidate', async () => {
      upsertEmbedding({
        ...
        body_fingerprint: 'Old issue Login page crashes special characters password',
      });
    ...
    it('similarity just below threshold -> not duplicate', async () => {
      upsertEmbedding({
        ...
        body_fingerprint: 'UI rendering CSS layout breaks flexbox container alignment overflow grid',
      });
