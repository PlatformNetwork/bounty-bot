...
describe("hmac", () => {
  ...
  describe("verifySignature", () => {
    it("valid round-trip (sign then verify)", () => {
      ...
      expect(verifySignature(body, signature, timestamp, testSecret)).toBe(
        true,
      );
    });
    ...
    it("rejects tampered body", () => {
      ...
      expect(
        verifySignature("tampered", signature, timestamp, testSecret),
      ).toBe(false);
    });
    ...
    it("rejects expired timestamp", async () => {
      ...
      const oldSig = createHmac("sha256", testSecret)
        ...
        .digest("hex");
      expect(verifySignature(body, oldSig, oldTimestamp, testSecret)).toBe(
        false,
      );
    });
    ...
    it("rejects NaN timestamp", () => {
      ...
      expect(
        verifySignature("test", signature, "not-a-number", testSecret),
      ).toBe(false);
    });

    it("rejects wrong-length signature", () => {
      const { timestamp } = signRequest("test", testSecret);
      expect(verifySignature("test", "tooshort", timestamp, testSecret)).toBe(
        false,
      );
    });
  ...
  describe("buildSignedHeaders", () => {
    ...
    it("signature in header matches signRequest output", () => {
      ...
      // Verify round-trip
      expect(
        verifySignature(body, rawSig, headers["X-Timestamp"], testSecret),
      ).toBe(true);
    });
  });
});
