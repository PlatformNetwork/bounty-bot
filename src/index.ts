...
import { normalizeIssue, shouldProcess, processIntake } from './validation/intake.js';
...
import { startRequeueRecovery, stopRequeueRecovery } from './queue/requeue-recovery.js';
...
export function createApp(): express.Express {
  ...
  apiRouter.post('/validation/trigger', (req, res) => {
    ...
    if (!issueNumber || typeof issueNumber !== 'number') {
      res.status(400).json({ error: 'bad_request', message: 'issue_number is required (number)' });
      return;
    }

    const parts = TARGET_REPO.split('/');
    if (parts.length !== 2 || !parts[0] || !parts[1]) {
      res.status(500).json({ error: 'config_error', message: 'Invalid TARGET_REPO' });
      return;
    }

    getIssue(parts[0], parts[1], issueNumber)
      .then((ghIssue) => {
        const normalized = normalizeIssue(ghIssue as unknown as Record<string, unknown>);
  ...
  apiRouter.get('/validation/:issue_number/status', (req, res) => {
    ...
    if (isNaN(issueNumber)) {
      res.status(400).json({ error: 'bad_request', message: 'Invalid issue_number' });
      return;
    }

    const status = getProcessingStatus(issueNumber);
    if (!status) {
      res.status(404).json({ error: 'not_found', message: `No bounty found for issue #${issueNumber}` });
      return;
    }
    ...
    handleRequeue(issueNumber, requesterId, requesterContext)
      .then((result) => {
        if (result.success) {
          ...
          res.status(422).json({ error: 'requeue_rejected', message: result.error });
        }
      })
      .catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : String(err);
    ...
    handleForceRelease(issueNumber)
      .then((result) => {
        if (result.success) {
          ...
          res.status(500).json({ error: 'release_failed', message: result.error });
        }
      })
      .catch((err: unknown) => {
        const msg = err instanceof Error ? err.message : String(err);
        logger.error({ issueNumber, err: msg }, 'Force-release endpoint failed');
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logger.warn({ err: msg }, 'Redis initialization failed — continuing without Redis');
  }
